...
// Esto asegura que estén disponibles para el HTML inyectado dinámicamente
...
window.editProject = (id) => {
  ...
  document.getElementById("in-priority").value = p.priority || "medium";

  // Actualizar selectores visuales de prioridad
  document
    .querySelectorAll(".prio-option")
    .forEach((o) => o.classList.remove("selected"));
  const prioOption = document.querySelector(
    `.prio-option[data-val="${p.priority || "medium"}"]`,
  );
  if (prioOption) prioOption.classList.add("selected");

  // Cargar datos temporales profundos (Clonación para no afectar original hasta guardar)
  tempTags = p.tags ? [...p.tags] : [];
  tempSections = p.sections ? JSON.parse(JSON.stringify(p.sections)) : [];

  // Renderizar sub-componentes del modal
  renderModalTags();
  renderModalSections();

  // Mostrar modal y ocultar menú dropdown
...
// Funciones del Drag & Drop de tareas (también globales)
let draggedItemIndex = null;
let draggedSectionIndex = null;
window.dragStart = (s, t) => {
  draggedSectionIndex = s;
  draggedItemIndex = t;
};
window.dragOver = (e) => e.preventDefault();
window.dropTask = (ts, tt) => {
  if (draggedSectionIndex === ts && draggedItemIndex !== null) {
    const item = tempSections[draggedSectionIndex].tasks.splice(
      draggedItemIndex,
      1,
    )[0];
...
window.updateTempTaskText = (s, t, val) => {
  tempSections[s].tasks[t].text = val;
};
window.addTempTask = (s) => {
  tempSections[s].tasks.push({ text: "", done: false });
  renderModalSections();
};
window.removeTempTask = (s, t) => {
  tempSections[s].tasks.splice(t, 1);
  renderModalSections();
};
window.removeTempSection = (s) => {
  if (confirm("¿Eliminar fase?")) {
    tempSections.splice(s, 1);
    renderModalSections();
  }
};
...
function renderProjectList() {
  ...
  filtered.forEach((p) => {
    ...
    const isFinished =
      (totalTasks > 0 && progress === 100) || p.status === "completed";
...
// --- RENDERIZADORES DE MODAL EXTRADOS ---
...
function setupModalLogic() {
  ...
  document.getElementById("btnOpenModal").onclick = () => {
    ...
    document.getElementById("btn-save-project").textContent =
      "Guardar Proyecto";
    form.reset();
    ...
    document.getElementById("in-start").value = new Date()
      .toISOString()
      .split("T")[0];
    ...
    document
      ...
      .forEach((o) => o.classList.remove("selected"));
    document
      .querySelector(`.prio-option[data-val="medium"]`)
      ?.classList.add("selected");
    document.getElementById("in-priority").value = "medium";
  ...
  form.onsubmit = (e) => {
    ...
    const projectData = {
      ...
      tags: [...tempTags],
      sections: JSON.parse(JSON.stringify(finalSections)), // Deep copy al guardar
